...
public class OxygenFillerPonder {
    ...
    public static void program(SceneBuilder sceneBuilder, SceneBuildingUtil util) {
        ...
        scene.overlay()
                .showControls(util.vector().blockSurface(util.grid().at(2, 2, 3), Direction.NORTH), Pointing.RIGHT, 40)
                .rightClick()
                .withItem(NorthstarItems.IRON_SPACE_SUIT_CHESTPIECE.asStack());

        scene.overlay()
                .showOutlineWithText(select.position(2, 2, 3), 40)
        ...
        scene.overlay()
                .showOutlineWithText(select.fromTo(4, 1, 2, 4, 2, 2), 80)
        ...
        scene.overlay()
                .showOutlineWithText(select.position(4, 1, 3), 80)
