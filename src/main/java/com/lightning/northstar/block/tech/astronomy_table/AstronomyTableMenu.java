...
public class AstronomyTableMenu extends AbstractContainerMenu {
    ...
    public void createResult() {
        ...
        if (item1_final_name.equals(item2_final_name) && item2_final_name.equals(item3_final_name)) {
            name_check = true;
        } else {
    ...
    private String targetGetter(String thing) {
        ...
        for (int i = 0; i < thing.length(); i++) {
            if (i > 6 && i < thing.length() - 2) {
                newthing += thing.charAt(i);
            }
        }
        return newthing;
    }
