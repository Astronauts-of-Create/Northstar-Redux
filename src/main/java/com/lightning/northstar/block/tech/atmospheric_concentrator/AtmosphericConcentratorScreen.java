...
public class AtmosphericConcentratorScreen extends AbstractSimiScreen {
    ...
    protected void init() {
        ...
        dimensionSelection.formatter(fluid -> fluid.asFluidStack(1).getDisplayName())
        ...
        saveButton.withCallback(() -> {
            onClose();
            NorthstarPackets.getChannel().sendToServer(new AtmosphericConcentratorEditPacket(be.getBlockPos(), dimensionSelection.get().asFluidStack(1)));
        });
