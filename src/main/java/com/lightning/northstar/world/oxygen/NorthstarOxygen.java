...
public class NorthstarOxygen {
    ...
    public static int getTankCapacity(Level level, ItemStack item) {
        return NorthstarConfigs.server().spacesuitBaseOxygen.get() +
               NorthstarConfigs.server().spacesuitAdditionalOxygen.get() * item.getEnchantmentLevel(level.holderOrThrow(AllEnchantments.CAPACITY));
    }
    ...
    public static void onBreathe(LivingBreatheEvent event) {
        ...
        if (isFullyCovered && !oxygenSource.isEmpty() && depleteOxygen(world, oxygenSource, world.getGameTime() % 20 == 0)) {
            event.setCanBreathe(true);
    ...
    public static boolean depleteOxygen(Level level, ItemStack stack, boolean deplete) {
        ...
        if (deplete)
            stack.set(NorthstarDataComponents.OXYGEN, Math.min(oxygen - 1, getTankCapacity(level, stack)));
        return true;
    }
