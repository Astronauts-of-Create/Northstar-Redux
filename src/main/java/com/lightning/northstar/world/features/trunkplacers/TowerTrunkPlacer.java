...
public class TowerTrunkPlacer extends TrunkPlacer {
       public static final Codec<TowerTrunkPlacer> CODEC = RecordCodecBuilder.create((p_226236_) -> trunkPlacerParts(p_226236_).and(p_226236_.group(IntProvider.POSITIVE_CODEC.fieldOf("extra_branch_steps").forGetter((p_226242_) -> p_226242_.extraBranchSteps), RegistryCodecs.homogeneousList(Registries.BLOCK).fieldOf("can_grow_through").forGetter((p_226234_) -> p_226234_.canGrowThrough), BlockStateProvider.CODEC.fieldOf("cap_provider").forGetter((p_161248_) -> p_161248_.capProvider))).apply(p_226236_, TowerTrunkPlacer::new));
               public final BlockStateProvider capProvider;
           private final IntProvider extraBranchSteps;
           private final HolderSet<Block> canGrowThrough;

           public TowerTrunkPlacer(int int1, int int2, int int3, IntProvider int4, HolderSet<Block> int7, BlockStateProvider cap) {
           ...
           public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> pBlockSetter, RandomSource pRandom, int pFreeTreeHeight, BlockPos pPos, TreeConfiguration pConfig) {
                  ...
                  this.placeBigCap(pLevel, pBlockSetter, pRandom, blockpos$mutableblockpos, pFreeTreeHeight, pConfig);
                  return list;
               }
           ...
           public void placeBigCap(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> pBlockSetter, RandomSource pRandom, BlockPos origin, int treeHeight, TreeConfiguration pConfig) {
               BlockPos.MutableBlockPos thing = new BlockPos.MutableBlockPos();
           ...
           protected boolean validTreePos(LevelSimulatedReader pLevel, BlockPos pPos) {
               return pLevel.isStateAtPosition(pPos, (state) -> state.is(Blocks.AIR) || state.is(NorthstarBlocks.TOWER_FUNGUS.get()) || state.is(NorthstarBlocks.VENUS_STONE.get()) || state.is(NorthstarBlocks.VENUS_DEEP_STONE.get()));
           }
        }