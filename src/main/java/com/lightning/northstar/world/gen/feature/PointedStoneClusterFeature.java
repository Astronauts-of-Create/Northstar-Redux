...
public class PointedStoneClusterFeature extends Feature<PointedStoneClusterConfiguration> {
    ...
    public PointedStoneClusterFeature(Codec<PointedStoneClusterConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<PointedStoneClusterConfiguration> pContext) {
        ...
        if (optional.isEmpty()) {
            ...
            PointedCrimsiteBlock.growPointedDripstone(levelaccessor, blockpos, optional.get(), i, false, config.stoneProvider().getState(randomsource, blockpos).getBlock());
            return true;
        }
    }
    ...
    private static void createPatchOfDripstoneBlocks(LevelAccessor pLevel, RandomSource pRandom, BlockPos pPos, PointedStoneClusterConfiguration pConfig) {
        Block block = pConfig.stoneProvider().getState(pRandom, pPos).getBlock();
        Block block2 = pConfig.dripstoneBlock().getState(pRandom, pPos).getBlock();
