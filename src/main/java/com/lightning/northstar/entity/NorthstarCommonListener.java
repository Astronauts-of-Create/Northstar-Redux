...
public class NorthstarCommonListener {
    ...
    public static void registerEntityAttributes(EntityAttributeCreationEvent event) {
        ...
        event.put(NorthstarEntityTypes.MARS_WORM.get(),
                Monster.createMobAttributes().add(Attributes.FOLLOW_RANGE, 16.0D).add(Attributes.MAX_HEALTH, 40.0D).add(Attributes.ATTACK_DAMAGE, 10).add(Attributes.MOVEMENT_SPEED, 0.2f).build());
        ...
        event.put(NorthstarEntityTypes.MERCURY_RAPTOR.get(),
                Monster.createMobAttributes().add(Attributes.FOLLOW_RANGE, 16.0D).add(Attributes.MAX_HEALTH, 36.0D).add(Attributes.ATTACK_DAMAGE, 9).add(Attributes.MOVEMENT_SPEED, 0.25f).build());
