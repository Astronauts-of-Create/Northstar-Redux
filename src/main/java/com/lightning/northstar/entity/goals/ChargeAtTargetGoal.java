...
//              System.out.println("SUBVEC: " + subVec);
            ...
            for (Entity colliders : this.chargingMob.level().getEntities(chargingMob, chargingMob.getBoundingBox())) {
                if (colliders instanceof LivingEntity lc) {
                    ...
                    lc.hurt(lc.level().damageSources().cramming(), 5);
            ...
            boolean xFlag = false;
            boolean zFlag = false;
            if (chargingMob.moveDirection.x > 0) {
                xFlag = subVec.x < 0;
            }
            if (chargingMob.moveDirection.x < 0) {
                xFlag = subVec.x > 0;
            ...
            if (chargingMob.moveDirection.z > 0) {
                zFlag = subVec.z < 0;
            }
            if (chargingMob.moveDirection.z < 0) {
                zFlag = subVec.z > 0;
...
//              System.out.println("distance: " +  blockPosToVec3(chargingMob.blockPosition()).distanceTo(blockPosToVec3(this.targetPos)));
            ...
            if (!chargingMob.passedTarget) {
                ...
                this.chargingMob.setDeltaMovement(delta.x + ((chargingMob.moveDirection.x / 50) * 0.7), delta.y + ((chargingMob.moveDirection.y / 50) * 0.7), delta.z + ((chargingMob.moveDirection.z / 50) * 0.7));
