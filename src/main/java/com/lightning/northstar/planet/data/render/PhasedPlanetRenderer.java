...
public record PhasedPlanetRenderer(
    ...
    public static final MapCodec<PhasedPlanetRenderer> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
    ...
    public void render(Level level, PoseStack pose, VertexConsumer vc, float size, Vector4fc color, Planet planet) {
        ...
        float s = size / 2f;
        vc.addVertex(matrix, -s, +s, 0).setUv(sprite.getU(u1), sprite.getV(v0)).setColor(color.x(), color.y(), color.z(), color.w());
        vc.addVertex(matrix, +s, +s, 0).setUv(sprite.getU(u0), sprite.getV(v0)).setColor(color.x(), color.y(), color.z(), color.w());
        vc.addVertex(matrix, +s, -s, 0).setUv(sprite.getU(u0), sprite.getV(v1)).setColor(color.x(), color.y(), color.z(), color.w());
        vc.addVertex(matrix, -s, -s, 0).setUv(sprite.getU(u1), sprite.getV(v1)).setColor(color.x(), color.y(), color.z(), color.w());
    }
