...
public class RocketFlameParticleData implements ParticleOptions, ICustomParticleDataWithSprite<RocketFlameParticleData> {
    ...
    public static final StreamCodec<ByteBuf, RocketFlameParticleData> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.VAR_INT, p -> p.posX,
            ByteBufCodecs.VAR_INT, p -> p.posY,
            ByteBufCodecs.VAR_INT, p -> p.posZ,
            RocketFlameParticleData::new
    );

    public static final MapCodec<RocketFlameParticleData> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
    ...
    @Override
    public MapCodec<RocketFlameParticleData> getCodec(ParticleType<RocketFlameParticleData> type) {
        return CODEC;
    }

    @Override
    public StreamCodec<? super RegistryFriendlyByteBuf, RocketFlameParticleData> getStreamCodec() {
        return STREAM_CODEC;
    }
