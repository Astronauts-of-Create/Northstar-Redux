...
public class DustCloudParticleData implements ParticleOptions, ICustomParticleDataWithSprite<DustCloudParticleData> {
    ...
    public static final StreamCodec<ByteBuf, DustCloudParticleData> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.VAR_INT, p -> p.posX,
            ByteBufCodecs.VAR_INT, p -> p.posY,
            ByteBufCodecs.VAR_INT, p -> p.posZ,
            DustCloudParticleData::new
    );

    public static final MapCodec<DustCloudParticleData> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
    ...
    @Override
    public MapCodec<DustCloudParticleData> getCodec(ParticleType<DustCloudParticleData> particleType) {
        return CODEC;
    }

    @Override
    public StreamCodec<? super RegistryFriendlyByteBuf, DustCloudParticleData> getStreamCodec() {
        return STREAM_CODEC;
    }
