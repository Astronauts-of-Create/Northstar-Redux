...
public class OxyFlowParticleData implements ParticleOptions, ICustomParticleDataWithSprite<OxyFlowParticleData> {
    ...
    public static final StreamCodec<ByteBuf, OxyFlowParticleData> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.VAR_INT, p -> p.posX,
            ByteBufCodecs.VAR_INT, p -> p.posY,
            ByteBufCodecs.VAR_INT, p -> p.posZ,
            OxyFlowParticleData::new
    );

    public static final MapCodec<OxyFlowParticleData> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
    ...
    @Override
    public MapCodec<OxyFlowParticleData> getCodec(ParticleType<OxyFlowParticleData> type) {
        return CODEC;
    }

    @Override
    public StreamCodec<? super RegistryFriendlyByteBuf, OxyFlowParticleData> getStreamCodec() {
        return STREAM_CODEC;
    }
