...
public class MarsEffects extends SpaceEffects {
    ...
    public void northstar$setupFogRender(ViewportEvent.RenderFog fog) {
        ...
        float fogThickness = minecraft.level.getRainLevel(minecraft.getPartialTick());
        if (fogThickness != 0) {
    ...
    public boolean tickRain(ClientLevel level, int ticks, Camera camera) {
        ...
        float rain_det = minecraft.level.getRainLevel(minecraft.getPartialTick());
        if (rain_det > 0) {
            float f = minecraft.level.getRainLevel(minecraft.getPartialTick()) / (Minecraft.useFancyGraphics() ? 1 : 2);
            if (!(f <= 0)) {
                    ...
                    if (biome.getPrecipitationAt(blockpos$mutableblockpos) == Biome.Precipitation.NONE) {
                        ...
                        if (j2 != k2) {
                            ...
                            if ((biome.warmEnoughToRain(blockpos$mutableblockpos))) {
                                ...
                                RenderSystem.setShaderColor(2, 1.178F, 0.698f, 0.5f);
                                int j3 = 0;
                                bufferbuilder.vertex((double) k1 - camX - d0 - 10.5D, (double) k2 - camY, (double) j1 - camZ - d1 + 10.5D).uv(0 + f2, 0).color(1, 1, 1, f4).uv2(j3).endVertex();
                                bufferbuilder.vertex((double) k1 - camX + d0 + 10.5D, (double) k2 - camY, (double) j1 - camZ + d1 - 10.5D).uv(1 + f2, 0).color(1, 1, 1, f4).uv2(j3).endVertex();
                                bufferbuilder.vertex((double) k1 - camX + d0 + 10.5D, (double) j2 - camY, (double) j1 - camZ + d1 - 10.5D).uv(1 + f2, 1).color(1, 1, 1, f4).uv2(j3).endVertex();
                                bufferbuilder.vertex((double) k1 - camX - d0 - 10.5D, (double) j2 - camY, (double) j1 - camZ - d1 + 10.5D).uv(0 + f2, 1).color(1, 1, 1, f4).uv2(j3).endVertex();
                            }
                        }
                    }
