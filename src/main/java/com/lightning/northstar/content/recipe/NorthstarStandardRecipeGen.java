...
public class NorthstarStandardRecipeGen extends StandardRecipeGen {
    ...
    ROCKET_THRUSTER = create(NorthstarBlocks.ROCKET_THRUSTER)
            .unlockedBy(() -> NorthstarItems.ROCKET_COMBUSTION_CHAMBER)
    ...
    SPACE_ATLAS = create(NorthstarItems.SPACE_ATLAS)
            .unlockedBy(() -> NorthstarItems.TITANIUM_INGOT)
            .viaShapeless(b -> b.requires(Items.BOOK)
                    .requires(NorthstarItems.TITANIUM_INGOT)),
    ...
    TITANIUM_TRAPDOOR = create(NorthstarBlocks.TITANIUM_TRAPDOOR)
            .unlockedByTag(() -> NorthstarItemTags.C_SHEETS_TITANIUM.tag)
            .returns(2)
            .viaShaped(b -> b.define('G', Items.GLASS)
                    .define('T', NorthstarItemTags.C_SHEETS_TITANIUM.tag)
                    .pattern("TGT")
                    .pattern("TGT")),
    ...
    private GeneratedRecipe createLamp(ItemProviderEntry<? extends ItemLike, ? extends ItemLike> lamp, TagHelper.Tag<Item> casing, Supplier<? extends ItemLike> core) {
        return create(lamp)
                .returns(4)
